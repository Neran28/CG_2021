...
public:
  ...
  std::optional<Vec3> raySphereIntersect(const Vec3& sphereCenter, const float& radius, const Vec3& rayOrigin, const Vec3& pixelPos) {
    ...
    const float b = Vec3::dot(rayDirection , (rayOrigin - sphereCenter) * 2.0f);
    const float c = Vec3::dot(sphereCenter,sphereCenter) + Vec3::dot(rayOrigin,rayOrigin) - 2.0f*Vec3::dot(rayOrigin,sphereCenter) - radius*radius;
    const float sqD = b*b - 4.0f*c;
    if (sqD < 0) return {}; // ray misses sphere
    const float D=sqrtf(sqD);
    const float t = (-b - D) * 0.5f;
    if (t > epsilon)
